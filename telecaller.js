...
function renderLeadsTable(rows) {
  ...
  rows.forEach(row => {
    ...
    // Extract telecaller name from email (e.g. john.doe@... -> John doe)
    const rawName = currentUserEmail ? currentUserEmail.split('@')[0].replace(/[^a-zA-Z]/g, ' ') : '';
    const senderName = rawName ? rawName.charAt(0).toUpperCase() + rawName.slice(1) : 'an Admin';
    ...
    let waMessage = encodeURIComponent(`Hello ${row.fullName || 'Student'}, this is ${senderName} from Thiranex regarding your registration for ${courseName}.`);
